...
void http_agent_t::printStartupInfo()
{
    ...
    std::cout << COLOR_CYAN << "\n"
              ...
              << "  |    GET    /api/v1/records         - With filters:                            |\n"
              << "  |           ?codec=h264             - Filter by codec                          |\n"
              << "  |           ?from_date=2026-05-01   - From date                                |\n"
              << "  |           ?to_date=2026-05-31     - To date                                  |\n"
              << "  |           ?file_path=video        - Filter by file path                      |\n"
    ...
    std::cout << COLOR_YELLOW << "\n"
              ...
              << "  |                                                                              |\n"
              << "  |    # Get records filtered by codec                                           |\n"
              << "  |    curl --noproxy \"localhost\" \"http://" << m_host << ":" << m_port << "/api/v1/records?codec=h264\" |\n"
              << "  |                                                                              |\n"
              << "  |    # Get records filtered by date range                                      |\n"
              << "  |    curl --noproxy \"localhost\" \"http://" << m_host << ":" << m_port << "/api/v1/records?from_date=2026-05-01&to_date=2026-05-31\" |\n"
              << "  |                                                                              |\n"
              << "  |    # Get record by ID                                                        |\n"
